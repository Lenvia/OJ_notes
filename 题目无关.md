...
但当遇到字符和字符串混合输入的时候可能会出现问题。因为cin读取的时候，会把回车键留在缓冲区。
...
**<font color=red>因此，平时如果使用cin字母或数字后，想要再输入字符串，需要使用getchar()或cin.get()读取字符末尾的回车。</font>**
...
稳定排序：在待排序的记录序列中，存在多个具有相同的关键字的记录，若经过排序，这些记录的**相对次序保持不变**，则为稳定排序。
